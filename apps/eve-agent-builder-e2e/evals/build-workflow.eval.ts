...
export default defineEval({
  ...
  async test(t) {
    ...
    const first = await session.send("BUILD_WORKFLOW_START");
    first.expectOk();
    t.messageIncludes("BUILD_WORKFLOW_ALLOCATED_OK");
    t.calledTool("agent_builder__workflow_allocate", { count: 1 });

    const ownerSwitch = await session.send("BUILD_WORKFLOW_OWNER_SWITCH", {
    ...
    const qaTest = await session.send("BUILD_WORKFLOW_QA_TEST");
    qaTest.expectOk();
    ...
    const reopened = await session.send("BUILD_WORKFLOW_REOPEN_FOR_EDIT");
    reopened.expectOk();
    t.messageIncludes("BUILD_WORKFLOW_REOPENED_OK");
    t.calledTool("agent_builder__workflow_reopen", { status: "completed", count: 1 });

    const revisedPm = await session.send("BUILD_WORKFLOW_REVISED_PM");
    revisedPm.expectOk();
    t.messageIncludes("BUILD_PM_REVISION_HANDOFF_OK");

    const revisedImplementor = await session.send("BUILD_WORKFLOW_REVISED_IMPLEMENTOR");
    revisedImplementor.expectOk();
    t.messageIncludes("BUILD_IMPLEMENTOR_REVISION_HANDOFF_OK");

    const revisedQaTest = await session.send("BUILD_WORKFLOW_REVISED_QA_TEST");
    revisedQaTest.expectOk();
    t.messageIncludes("BUILD_QA_REVISION_TEST_REQUEST_OK");

    const revisedTest = await session.send("BUILD_WORKFLOW_REVISED_TEST");
    revisedTest.expectOk();
    t.messageIncludes("BUILD_REVISION_TEST_EVIDENCE_OK");

    const revisedApproval = await session.send("BUILD_WORKFLOW_REVISED_QA_APPROVE");
    revisedApproval.expectOk();
    t.messageIncludes("BUILD_QA_REVISION_APPROVED_OK");

    const refused = await session.send("BUILD_WORKFLOW_PUBLISH_WITHOUT_APPROVAL");
    ...
    const nextTurn = await session.send("BUILD_WORKFLOW_NEXT_TURN");
    nextTurn.expectOk();
