...
for (const name of expectedChildren) {
  ...
  assert(child.sandbox === null, `${name} inherited a root sandbox`);
  ...
  assert(
    JSON.stringify([...child.disabledFrameworkTools].sort()) ===
      JSON.stringify(requiredDisabled),
    `${name} did not explicitly disable every unmounted Eve framework tool`,
  );
  assert(child.dynamicTools?.length === 1, `${name} lacks its one role-bound dynamic tool mount`);
...
console.log(`Verified Eve 0.38 compiled isolation for ${expectedChildren.join(", ")}.`);
