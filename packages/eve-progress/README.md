...
```sh
pnpm add eve@0.63.0 eve-progress
```

This package requires Eve 0.63.0. At each session and turn boundary, the extension
captures Eve's public dynamic-resolver channel metadata into session state before
hook dispatch. This supports Eve 0.63.0's intentionally smaller hook context
