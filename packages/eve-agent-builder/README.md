...
owner-scoped saved agents in Eve 0.54.3. PR 04 adds a durable PM → implementor →
...
additional response validation Eve 0.54.3 does not expose to dynamic approval
...
child is reused only inside that parent turn; Eve 0.54.3 does not refresh
...
An unknown Eve `agentId` starts a fresh child in Eve 0.54.3. That child has no
...
Eve 0.54.3 has no public binding from an `ask_question` answer to a later call,
...
The package targets `eve@0.54.3`, Node.js 24 or newer, strict TypeScript, Zod
