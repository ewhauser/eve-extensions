...
```sh
pnpm add eve@0.49.0 eve-openai-connectors
```

### Install the required Eve patch

Client-executed and provider-native deferred discovery require a small patch to Eve 0.49.0. The package ships the exact patch used by this monorepo, but pnpm does not apply patches from dependencies automatically. Copy it into your application:
